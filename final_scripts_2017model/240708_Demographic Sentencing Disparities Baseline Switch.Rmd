...
data <- read_csv(here::here("data/io_2002_2023.csv")) %>% 
  filter(SOURCES==1) #Ryan said the Commission does this--but the data seems to have been prefiltered, because it's all ==1

postprotect <- data %>% 
  filter(postprotect) #06-24-04

postbooker <- data %>% 
  filter(postbooker) #dates are not perfect bc case came down 12-10-2007
...
This document attempts to replicate the Commission's "refined model" for estimating demographic disparities in sentencing. For more information about the model and for the reference regressions, see "Demographic Disparities Baseline Switch Reference Docs."

The data runs from the beginning of the PROTECT Act period through to the end of FY 2023. 
